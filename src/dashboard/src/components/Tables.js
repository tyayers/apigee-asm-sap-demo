...
export const TransactionsTable = (props) => {
  ...
  //var showMapVariable = window.location.href.split('showMap=')[1] == "true" ? true : false;
  ...
  // var es = new EventSource('https://transport-service-h7pi7igbcq-ey.a.run.app/transport/alerts/stream');
  // es.onmessage = function (event) {
  //   refreshData();
  //   var mapElem = document.getElementById('maps-frame');
  //   if (mapElem) mapElem.src += '';
  // };

  function connect() {
    window.WebSocket = window.WebSocket || window.MozWebSocket;

    var connection = new WebSocket('wss://test.apigee.theapishop.com/broadcastservice');
  ...
  useEffect(() => {
    //setAppState({ totalTransactions: 0, transactions: [] });
    // fetch("/dashboard/parameters")
    // .then((res) => res.json())
    // .then((parametersResult) => {
      //setShowMap(parametersResult.parameters.showMap == "true");
    if (appState.totalTransactions == 0) {
      //dataLoaded = true;
      refreshData();
    }
    //});
  ...
  const refreshData = () => {
    ...
    fetch(orders)
      ...
      .then((ordersResult) => {
        //var showMapVariable = window.location.href.split('showMap=')[1] == "true" ? true : false;
        setAppState({ totalTransactions: ordersResult.orders.length, transactions: ordersResult.orders });
        for (var i=0; i<ordersResult.orders.length; i++) {
          if (ordersResult.orders[i].Latitude) {
            setShowMap(true);
            break;
          }
        }
  ...
  const TableRow = (props) => {
    const { SalesOrder, CreatedByUser, CreationDate, TotalNetAmount, OverallTotalDeliveryStatus } = props;
    const statusVariant = OverallTotalDeliveryStatus === "DELIVERED" ? "success"
      : OverallTotalDeliveryStatus === "FULFILLMENT" ? "warning"
        : OverallTotalDeliveryStatus === "SCHEDULED" ? "danger" : "primary";
    ...
    <div>
      ...
      <Card border="light" className="table-wrapper table-responsive shadow-sm">
        <Card.Body className="pt-0">
          <iframe id="maps-frame" src="/dashboard/map.html" style={{width: "100%", height: "600px"}}></iframe>
        </Card.Body>
      </Card>
      }
      <br></br>      
      <Card border="light" className="table-wrapper table-responsive shadow-sm">
        <Card.Body className="pt-0">
          <Table hover className="user-table align-items-center">
            ...
            <tbody>
              {appState.transactions.map(t => <TableRow key={`transaction-${t.SalesOrder}`} {...t} />)}
            </tbody>
          </Table>
          <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
            ...
            <small className="fw-bold">
              Showing <b>{appState.totalTransactions}</b> out of <b>25</b> entries
            </small>
          </Card.Footer>
        </Card.Body>
      </Card>
